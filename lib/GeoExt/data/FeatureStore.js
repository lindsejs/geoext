...
GeoExt.data.FeatureStoreMixin = function() {
    ...
    return {
        ...
        addFeaturesToLayer: function(records) {
            var i, len, features,add,j;
            features = [];
            len=records.length;
            var layerFeatLen =  this.layer.features.length;
            for(i=0; i<len; i++) {
            	 //add only if already not added to layer
            	var f = records[i].getFeature();
            	add = true;
            	 for(j=0; j<layerFeatLen; j++) {
            	 	if(this.layer.features[j]==f){
            	 		 add = false;
            	 		break;
            	 	}
            	 }
